    ...
    while tbool:
        ...
        tempStr += (inputf(second_name) +
                    inputf(int(input("Оценка по матиматике: ")), int(input("Оценка по Русскому: ")),
                           ...
                           func=lambda x: 0 <= x <= 5))
        print()
        arrStudentFullInfo[j + 1] = tempStr
        arrStudentInfo [tempStr[0]] = tempStr[1] + round(statistics.mean(tempStr[2:]), 2)
        j += 1
    print("Студент \t| Средний бал")
    for i in arrStudentInfo.keys():
        print(f"{i} \t\t| {arrStudentInfo[i]}")

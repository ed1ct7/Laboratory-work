# Вторая задача на классы с наследованием из ЛР 8-10.
...
def main():
    ...
    goods_type_arr = ["техника", "мебель", "молоко", "i9 14900K"]
    ...
    class Sales:
        ...
        def change_goods_type(self):
            try:
                print("Виды товаров: ")
                for i in range(len(goods_type_arr)):
                    print(f"Товар {i + 1} - {goods_type_arr[i]}")
                self.goods_type = int(input("\nВведите номер товара: "))
                # Проверка на действительность выбора
                while not (1 <= self.goods_type <= len(goods_type_arr)):
    ...
    if type(good.best_month()) == list:
        print("\nЛучшие месяцы: ")
        print("Прибыль лучших месяцев - ", good.month_profit[good.best_month()[0]])
